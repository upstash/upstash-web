...
First, we load the `taskIds`, which are stored in [a Redis Set](https://redis.io/docs/data-types/sets/). Then we use each ID to fetch the corresponding task. [A pipeline](/docs/redis/sdks/ts/pipelining/pipeline-transaction) ensures we send all get commands in one request to Upstash.
