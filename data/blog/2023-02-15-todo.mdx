...
In a previous article, we discussed [how we can use Redis to store the state of your user's quota](/blog/quota-based-saas) on a full-stack Next.js app. Doing this allows us to improve the performance of our application, as a Redis instance is much faster than a database.

However, a Redis server is volatile. If keys expires or the server crashes, we will lose the state of our users' quota. To prevent this, we can use QStash to sync the state of our Redis server to a database.
...
- `QSTASH_TOKEN`: authentication token sent to our Next.js CRON job endpoint, by QStash.
