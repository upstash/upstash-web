...
## 9. Freestyle: full Linux VMs for long-running agents

[Freestyle](https://www.freestyle.sh) is the opposite of the quick "run this snippet" sandboxes: it gives your agent a full, hardware-virtualized Linux VM with its own kernel, nested virtualization (Docker, even KVM inside the VM), and full Linux networking with real root access.

It is also built to run for a long time. VMs are persistent, cost nothing while paused, and resume in under 100ms. Snapshots capture the full machine state, and a running VM can be forked in milliseconds without pausing it, so taking snapshots often is cheap. VMs can go up to 64 GB of RAM on demand.
...
const result = await vm.exec({ command: "uname -a" });
console.log(result.stdout);

// grow the same VM as the task grows
await vm.resize({ cpu: 8, memory: 16, storage: 80 });
```

It also comes with Freestyle Git, an API-first git layer with webhooks and GitHub sync that can clone directly into a VM.

**When to use it:** when your agent runs for a long time and needs a real computer, root, its own kernel, Docker, custom networking, rather than a short-lived code runner.
...
**What's your hardest constraint?** Strong isolation → **E2B** or **Fly.io**. A GPU in the box → **Modal** or **Northflank**. Cold-start speed → **Daytona**. A long-lived VM with root → **Freestyle**. Your existing ecosystem → **Cloudflare** or **Vercel**. Coding-agent tooling and compliance → **Runloop**. Self-hosting → **Northflank**.
