...
Welcome to this blog post, where we embark on a journey into the inception and evolution of the innovative [DegreeGuru](https://degreeguru.vercel.app/) chatbot application utilizing *Retrieval Augmented Generation (RAG)* approach. Here, we delve into the compelling motivations that sparked its creation, tracing its development through the intricate web of tools such as [Upstash Vector](https://upstash.com/docs/vector/overall/getstarted), [Upstash Redis](https://upstash.com/docs/redis/overall/getstarted), [Langchain](https://js.langchain.com/), [Next.js](https://nextjs.org/), [Vercel](https://vercel.com/), and the [Vercel AI SDK](https://sdk.vercel.ai/docs).

Following the exploration of our motivations, we pivot towards discussing our approach and the requirements that guided our development process. Subsequently, we provide an overview of the tech stack employed. In the ensuing sections, we delve into the implementation of the crawler and offering insights into the inner workings of the DegreeGuru app.

# Motivation

Consider a future master's student, eagerly exploring various university programs. Picture her quest for vital details: application deadlines, submission methods—online or by mail, admission criteria, and program specifics.

However, the websites of different universities seem worlds apart. They vary immensely in design and how information is organized, making the search for required details a tedious task.

Imagine if there were a simpler method to access information on these websites! Enter **DegreeGuru**: a chatbot that can be swiftly tailored to **any website**, **in under an hour**. Including this innovative tool in the university webpages will empower our master's student, swiftly providing her with the specific information she seeks, all through a user-friendly chat interface.
...
1. **Simple Interface**: One of the primary goals of DegreeGuru is to offer users a straightforward and user-friendly interface.
2. **Streaming Responses**: Instead of waiting for the Large Language Model (LLM) to generate the entire response before sending it to the user, we stream tokens as they are generated using Vercel AI SDK. This means users get to see partial responses in real-time, creating a more dynamic and interactive conversation flow.
3. **Reliable Source Attribution**: Answers provided by DegreeGuru should include links to the sources of the information. This approach ensures transparency and reliability, allowing users to verify the information themselves.
4. **Data Collection**: To populate the Upstash Vector Store with relevant information, we have developed an easy-to-configure scraper. The scraper splits the page contents into chunks as it crawls them and stores them in the Upstash Vector Store after fetching their vector embeddings.
5. **Configurability**: DegreeGuru project is designed to be domain agnostic, meaning it can be easily configured for use on any website, not limited to universities. This requirement ensures that the chatbot can adapt to various contexts and serve diverse purposes beyond educational institutions.
...
### Vector Store
We rely on the [Upstash Vector](https://upstash.com/docs/vector/overall/getstarted) for storing vectors and metadata. Setting up an Upstash Vector Store is a breeze, akin to flipping a switch: Simply head to the [Upstash Console](https://console.upstash.com/), access your account, select "Vector" from the navigation bar, and create an index with the click of a button labeled "Create Index". For further instructions, delve into the ["Configure Environment Variables" section of our guide](https://github.com/upstash/DegreeGuru/tree/master?tab=readme-ov-file#crawler).
...
In the realm of conversational AI, the Retrieval Augmented Generation (RAG) approach has emerged as a powerful technique. RAG combines the strengths of both retrieval-based and generative models, offering a nuanced solution for chatbots. At its core, RAG leverages a retriever to efficiently search through a vast amount of data. This retrieved information then augments the generative model's ability to produce accurate responses. In our project, we implement RAG with the Upstash Vector serving as the retriever.

### Rate Limiting

Rate limiting in the context of the chatbot serves as a crucial cost management strategy. Each interaction with the chatbot incurs expenses, not only through the utilization of OpenAI credits but also by triggering Upstash Vector, which follows a [pay-per-usage model](https://upstash.com/docs/common/concepts/scale-to-zero#pay-for-usage).
...
In the development of any chatbot, defining the agent template is a critical step towards ensuring an effective user experience. This template serves as the backbone of how the chatbot interacts with users, outlining its persona, tone, and core functionalities.

In our agent template, the bot is designated as DegreeGuru, specializing in answering inquiries about Stanford University. The template instructs the bot to respond exclusively to questions based on available context, and encourages the use of relevant links whenever feasible.
...
Once you've finished populating your vector store, deploying the chatbot with Vercel is a breeze. Should you choose to crawl a source other than Stanford, we suggest modifing our [agent template](https://github.com/upstash/DegreeGuru/blob/master/src/app/api/guru/route.tsx#L105) accordingly. This can be done by forking our repository, making the necessary updates to the template, and finally clicking the "Deploy" button located at the top of the README file within your forked repository to launch the app on Vercel.

# Conclusion

In conclusion, DegreeGuru stands as a testament to the power of innovative technology when harnessed for practical solutions. Through the integration of tools such as Upstash Vector, Redis, Langchain, Next.js, and Vercel, we have crafted a useful proof of concept. The DegreeGuru, utilizing the Retrieval Augmented Generation (RAG) approach, empowers users with swift access to information, all within a user-friendly chat interface.

The ability to customize DegreeGuru to any website in under an hour opens doors for universities, businesses, and individuals alike. Imagine the ease with which a prospective student can now navigate the vast web of university program details, deadlines, and admission criteria. This same ease of access can be seamlessly offered to the users of any website, using the DegreeGuru project.
