...
We define a tool called `translate` with a schema that specifies what parameters it accepts: a `text` string and a `targetLanguage` (limited to Spanish, French, or German).
