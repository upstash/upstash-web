...
Now, create a `.env` file at the root of your project. You are going to use the `AUTH_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` values.
...
In the code above, an instance of Edge-compatible Redis client is exported to be used throughout the application.
