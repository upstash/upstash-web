...
For this blog post, we'll make a few assumptions before continuing, but ideally, you should have:
...
Have you ever wanted to generate your own stories using AI? With OpenAI's completions API and Upstash's QStash and Redis, it's now easier than ever to create your own custom stories using natural language processing. In this tutorial, we'll walk you through the process of setting up and using these tools to generate unique and engaging stories.
...
You're likely to get a decent understanding of how the app is set up from looking through the code, but as a bit of a higher-level overview, there is the image below showing some parts of the application flow and how they communicate.

![App Flow](https://user-images.githubusercontent.com/5033092/210117420-ba0f8c26-cc36-4fd3-b2fc-05cb90ff2715.png)

## Project Setup

First up, we'll want to create a Next.js project. This can be done by running the following to create a new Next.js project with TypeScript. You can find out the steps to set up Next.js [here](https://nextjs.com).
...
You'll now want to create a `.env.local` file and populate it with the following keys (and the values from the relevant sources).
...
We first check that the request method is `POST`, and send a response with a status code of 400 (indicating a client error) if it is not. We then proceed to destructure the theme, character, and moral fields from the body of the request.
...
First off, we try to decode the body of the incoming request, which will be a base64-encoded string, and if successful, store the decoded string in Redis under the same key as what was returned when we sent the initial request to QStash.
...
First up, we destructure the `id` from the query object of the request. We then try to retrieve the data stored in Redis under the destructured `id`, and if no data is found, send a response with a status code of 404 (indicating that the requested resource could not be found) and a message stating so.
...
- Update the frontend styling to be a lot more visually appealing and colorful
