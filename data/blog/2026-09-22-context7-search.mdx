...
description: "Ground your coding agent in official documentation with one REST request. Send a question and get back verified, attributed, token-efficient snippets."
draft: false
---

We're opening up the Context7 database.

Coding agents guess. They call APIs that changed two versions ago, use options that never existed, and copy patterns from an old forum thread. The fix is grounding: before the agent writes code, it searches the real documentation and works from that. Search engines like Exa already play this role for agents on the open web. Context7 Search is the grounding API for code.
...
Think of it as **Exa for code**: a search engine built for coding agents, with results from official docs only.
