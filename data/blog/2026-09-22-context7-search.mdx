...
Coding agents guess. They call APIs that changed two versions ago, use options that never existed, and copy patterns from an old forum thread. The fix is grounding: before the agent writes code, it searches the real documentation and works from that. Search engines like Exa already play this role for agents on the open web. Code needs the same thing, but with a better source.

That source is Context7. It indexes documentation from thousands of libraries, frameworks, and APIs, published and maintained by the library owners. Until today, the only way to use it was a two-step API built for looking up one library at a time. Now there's a single search endpoint: you send a question, and we take care of the rest.

Think of it as **Exa for code**: a grounding API built for coding agents, with results from official docs only.
