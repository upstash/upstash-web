...
- `limit`, which returns an object containing `is_allowed` field and some additional metadata about an `identifier`'s request
...
```python
remaining = ratelimit.get_remaining("identifier") # Returns the number of requests left for the given identifier.
reset = ratelimit.get_reset("identifier") # Returns the UNIX timestamp in seconds when the remaining requests will be reset or replenished.
```
## Closing words
