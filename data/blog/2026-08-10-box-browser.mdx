...
<Summary>
  ...
  - Drive it three ways: raw Playwright/CDP, single natural-language actions, or a fully autonomous agent.
...
The same browser answers to three levels of control, and you can mix them on the same tab.
...
**One action at a time.** `act()` resolves and executes a single instruction against the live page. Good for flows where your code decides each step.

```typescript
await tab.act("dismiss the cookie banner")
```

**Fully autonomous.** `run()` reads the page, acts, and repeats until the task is done or it hits the step limit. Pass a schema to get structured data back at the end.

```typescript
import { z } from "zod"

const { data, completed } = await tab.run(
  "Find the pricing page and summarize the free tier",
  { schema: z.object({ summary: z.string() }) },
)
```

Because they all drive the same tabs, you can script the brittle parts, like login and pagination, with Playwright, then hand the tab to `act` or `run` for the fuzzy parts.
