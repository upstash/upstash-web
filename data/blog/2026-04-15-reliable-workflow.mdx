---
title: "Making Upstash Workflow And QStash Reliable"
...
Our first implementation used **Redis Streams** and performed linear scans on queries. That was workable initially, but it became a bottleneck very quickly.

We moved from there to **SQLite**, which improved the situation for a time. But as data volume continued to increase, SQLite also became a limiting factor for the kinds of queries and scale we needed to support.

Eventually, we migrated again, this time to **Elasticsearch**.
