...
[LaunchDarkly](https://launchdarkly.com/) is a feature management tool for software applications. It provides feature flags to development teams to control their software product releases and tests.

- **Testing code live in production**

  Teams can create groups of users according to their specific identifiers, such as their unique keys, regions, users’ ages, etc.

  Test groups can also be created according to the users’ identifiers for testing purposes.

  Even if teams don’t want to classify their users, they can deploy their tests to just some percentage of users by enabling the feature flags for x% of users.

- **Feature release management**

  The new deployment, new feature, or any additional thing in the software product can be controlled by a feature flag.

  For example, a team wanted to create a new feature for users in a specific region, let’s say in the USA.

  The thing that the team needs to release their new feature in this country is to create a feature flag, which acts as a controller for the new feature. By creating rules for this feature flag through a simple UI, teams can adjust their targets for their new feature.

The benefits of LaunchDarkly do not end here. You can check their [website](<[https://launchdarkly.com/](https://launchdarkly.com/)>) and [use cases](<[https://launchdarkly.com/use-cases/](https://launchdarkly.com/use-cases/)>) for further information.

LaunchDarkly provides a Redis® integration to enable teams to have a Redis-backed feature store. At that point, Serverless Redis® comes into play as an easy, fast and feasible option.

### **LaunchDarkly Setup**

The first step is to create a personal or team account in [LaunchDarkly](<[https://app.launchdarkly.com/](https://app.launchdarkly.com/)>)
...
To keep things simple in this demo, I am going to create a feature flag named “test,” and this flag will return a boolean value. So, the variations of this flag will be true or false.
...
The users may also include other development teams in the company. If one of the teams doesn’t want to affect some teams in the same company, then they can prevent them to have the new feature by returning false to those teams.

Another option is creating rules for the return values of the feature flag. It is the same logic with an “if” statement.
...
One way that I loved most to adjust the distribution of the flag was the percentage rollout. By this option, we can return true to the specified percentage of the users, and false to others.
...
After that, our LaunchDarkly Java client will look like the following.
...
I created the LaunchDarkly Java client like this because of the warning in their docs.
...
In this blog, I created my API endpoint using [Spring](<[https://start.spring.io/](https://start.spring.io/)>) with which I feel comfortable.
...
Since this was just an example demonstrating the integration, the functionality was very basic. LaunchDarkly can be used in so many ways and serverless Redis is ready to store flags.
