...
The benefits of Launchdarky do not end here. You can check their [website](https://launchdarkly.com/) and [use cases](https://launchdarkly.com/use-cases/) for further information.
...
The first step is creating a personal or team account in [LaunchDarkly](https://app.launchdarkly.com/)
...
We are going to follow the instructions on [LaunchDarkly docs](https://docs.launchdarkly.com/sdk/server-side/java#getting-started).
...
In this blog, I created my API endpoint by using [Spring](https://start.spring.io/) which I feel comfortable with.
...
We are following the [Upstash docs](/docs/redis) first, if we don’t have a Redis database yet.

Then, to connect our serverless Redis to the LaunchDarkly client, first we need to install the LaunchDarkly Java Redis library, as it is stated in their [library](https://github.com/launchdarkly/java-server-sdk-redis).
...
When we check our Upstash Redis by using the “Data Browser” tab in the [console](https://console.upstash.com/), we can see the following data.
...
In this blog post, we have utilized feature flags from [LaunchDarkly](https://launchdarkly.com/) and stored them in Upstash Redis by using the LaunchDarkly Redis integration.
