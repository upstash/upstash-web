...
- A [Replicate](https://replicate.com) account with access to your API token
...
Now that we have a basic Next.js project set up, we can install Upstash's Redis library by running the command:
...
To begin with, we'll need a form that handles the submission, polling, and displaying of completed images.
...
Once this request has been sent to the API and a response has been received with the prediction information, the component enters into a polling state that sends a **GET** request to `/api/poll` once per second to check if the prediction has completed yet. Once a polling request returns a successful response, indicating that Replicate has sent a request to our callback endpoint, we'll now have access to the prediction output.
...
Replicate has a variety of models available that can be used via an API. With Vercel and Upstash, it's easier than ever to utilize machine learning models and deploy usable web applications.

If you'd like to view the complete repository, you can access it [here](https://github.com/andreaselia/photorescue).

## Further Development

This is just a simple example of utilizing a rather simple model with Replicate. By simply switching up the form parameters and the version in the API, you can easily change to another model as long as you have your Replicate API token linked, you'll be able to consume any of the available models.
