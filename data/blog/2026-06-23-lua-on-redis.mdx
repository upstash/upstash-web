...
its so easy and fast, i honestly can't imagine building without it now. i just wish it was better at time series data 😭

also i swear upstash didnt make me say this](https://cdn.bydefault.so/nNigEAWk7VwUkpz0wIsm-.png)
...
```typescript
const [count, next] = await redis.multi()
  .get<number>("count")
  .incr("count")
  .exec();
```
...
## When should I use EVAL\_RO instead of EVAL?

Use EVAL\_RO for a script that only reads. It runs Lua the same way EVAL does, but Redis rejects any write command inside it, so it can run on a read replica and a stray write turns into an error instead of a silent change.
...
The @upstash/redis SDK exposes this as `redis.evalRo(script, keys, args)`, and createScript takes a `{ readonly: true }` option that gives you the same EVALSHA-first behavior with the read-only variants. If a script is meant to be read-only, EVAL\_RO makes Redis enforce that.
