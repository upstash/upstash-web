...
**Direct access.** `ARGET myarray 47` is a lookup, not a walk. It costs the same at index 47 and at index 47,000,000. For random reads and writes, this makes arrays several times faster than lists in Redis's own benchmarks, and the gap grows with the size of the list.
