...
Every step is a real workflow step, so you can stream what the agents are doing.
The demo uses [Upstash Realtime](https://upstash.com/docs/workflow/howto/realtime/basic)
to push each agent's plan, tool calls, and final answer to the browser as they
happen, so you can watch the orchestrator hand off to the researcher and the
writer. The UI is in the
[example app](https://github.com/upstash/workflow-js/tree/main/examples/agent-workflows).
