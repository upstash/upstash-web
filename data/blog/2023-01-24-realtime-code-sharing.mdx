...
As Bytecrowds uses [Next.js](https://nextjs.org), a solid choice for integrating GitHub OAuth was [Auth.js](https://authjs.dev). Fortunately, we could also leverage the [Upstash adapter](https://authjs.dev/getting-started/adapters/upstash-redis) for storing sessions, allowing us to use Redis® all across our app.
...
Want to see Bytecrowds in action? Go to Bytecrowds website, click on the "new bytecrowd" button and open the link in multiple tabs. When you start writing, you'll see that the code is replicated across all of the connected clients.
