...
// 👇 Managing trial subscription workflow
...
//  It invokes the onboarding and trial workflows
const registerUser = createWorkflow(async (context: WorkflowContext<{userList: string}>) => {
...
Check out the [documentation](https://upstash.com/docs/workflow/rest/runs/runs) for more details.
...
They had to open a local-tunnel and use that address to trigger workflow runs. Although this approach remains very useful, we decided to distribute a development server for our users.
