...
In many SaaS applications, you may have encountered a quota system. It is a system that limits the number of actions a user can do in a given time.
...
These API requests are used to retrieve the content of a spreadsheet and turn it into JSON. This is what [fastsheet](https://fastsheet.co) does, turn any Google Sheets spreadsheet into a JSON API.

![Example of fastsheet's quota system](/blog/saas-quota-fastsheet/fastsheet-quota.png)

## Defining the database schema

As mentioned earlier, we will use Prisma as an ORM to interact with the database. Here is an example of a `User` model which implements a quota system, as well as its `Spreadsheet` model to store _spreadsheets_:
...
- A `User` can have multiple `Spreadsheets` linked to it.
...
- This code is harder to make **Edge compatible**; you need to create multiple database replicas around the world _(not an easy task!)_.
...
We used it to implement a quota system, but the original idea can be applied to many more use cases, such as caching more database queries or caching the results of an API request.

The final goal of caching is to make your API faster and more likely to consume fewer resources _(e.g., Planetscale database read/write queries)_.
...
If you want to see this kind of optimization live, you can check out [**fastsheet**](https://fastsheet.co?rel=upstash), an API service that turns your Google Sheets into an API in a few clicks. It's free to use with a generous free tier.
