---
slug: serverless-redis-streams
title: "Introducing Redis Streams Beyond Memory"
author: sancar
tags: [redis, announce, streams]
---

Redis® Streams has become one of the most anticipated features that we implemented so far. 
We have finished the implementation and we are ready to ship. Also, we are excited about telling the differences between 
**Redis® Streams by Upstash** and the standard Redis.

## What is a Redis® Stream 

A Redis® stream is an append-only log. Each entry added to a log has a unique id (`millisecondsTime-sequenceNumber`), and 
...
If you are familiar with Apache Kafka, Redis® Streams is essentially the same. The differences are in the details.
...
If you already decided to use Apache Kafka instead, see our getting started guide for [Apache Kafka on cloud](https://docs.upstash.com/kafka).

This is how Redis® Streams looks in action. A user tracking system can store data to a stream as follows:
...
##  Better Redis® Streams By Upstash
...
While implementing Redis® Streams, we wanted to address the real use case of a streaming API. Streaming data tends to be big and never-ending.
...
## How limitless Redis® Streams is implemented

To be able to store much more data than that can fit into memory, we decided to implement disk-based Redis® Streams. 
...
XREAD or XRANGE is requested. This way, you can keep all the data you need in Redis® Streams without worrying.

## Conclusion

We are excited to give you Redis® Streams without the limit of memory. 
Enjoy limitless Redis® Streams without worrying how much it will cost you.
