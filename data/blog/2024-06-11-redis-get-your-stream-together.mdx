...
I will use the same commands as an example that I shared in [the bug report](TODO sancar put link here) on the Redis repo.
...
Then, we delete one of the already read items.
...
If we look at the stream after the deletion, for the group to come to 0-2, it should read only 1 item, not 2. But we are still saying it is valid. If we check the lag calculation, it still checks out.
