...
In QStash, you can retrieve your queue and check the `lag` with the following [API](https://upstash.com/docs/qstash/api-reference/queues/get-a-queue). One of our careful users reported that this lag value is not correct. QStash lag and Redis Stream lag are not the same. In the initial release of the queue, we provided the stream lag as it is. This is on us; we should have been more careful. Let me try to show the difference.
