...
For this chat application, we will use the Pub/Sub channels of Ably to let users send messages by "publishing" messages to an Ably channel and let users receive the messages sent to the channel by "subscribing".

Ably provides these Pub/Sub channels by adding an abstraction layer on top of Web Sockets with additional features. Some of them are as follows:
...
We don't need to create the channel. A channel is created once someone publishes something to it.

As a preliminary step before implementing messaging functionality, we need to get a username from the user. We will make it very simple since authentication is not the focus of this blog.
...
Lastly, we will retrieve the chat history when the page is first loaded.
...
The server in this demo application will subscribe to the Ably channel, push it to the Upstash Redis database, and return chat history from Upstash Redis when a client requests.

We will first configure the server in the "app.js" file.
...
Next, we will create our server in "server.js," subscribe to the Ably channel, and connect to Upstash Redis.
...
The next step is pushing the incoming messages to the Upstash Redis database. We will perform this operation with an Ably channel subscription.
...
Once you enter your username, you will be able to open the chat.

If you send messages from one tab and open `localhost:3000` in another tab using a different username, you can see the messages sent from the previous tab.
...
Thanks to Upstash Redis, you will retrieve the chat history every time you open the chat application.
...
In this blog post, we used Ably to build a real-time chat application using its Pub/Sub channels. While doing so, we stored the messages in the Upstash Redis database. These two tools made this application easy to build and fast.
