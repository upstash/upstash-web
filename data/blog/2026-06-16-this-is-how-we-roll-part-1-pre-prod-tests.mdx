...
slug: this-is-how-we-roll-part-1-pre-prod-tests
authors: [burak]
tags: [redis]
...
By the time a tag has been pushed and validated, the DynamoDB record for that commit looks something like this:

```json
{
  "commit_id":         "a3f8d12b9e1c4f7a2d5e8b0c3f6a9d2e5b8c1f4a",
  "status":            "success",
  "timestamp":         "2025-01-15T14:32:07Z",
  "flow_run_url":      "https://github.com/upstash/upstash-cloud/actions/runs/12345678",
  "release_tag":       "v4.56.43",
  "release_timestamp": "2025-01-15T16:45:22Z"
}
```

The `commit_id` was written when the CI run finished. The `release_tag` and `release_timestamp` were added later, when the tagging workflow ran and confirmed the status was clean.
...
The hook queries DynamoDB using the `release_tag` as a lookup key. If the entry is missing, or the status is anything other than `"success"`, it exits 1 and prints the workflow URL:
...
![The prepare hook queries DynamoDB by release tag before Helm touches a single pod; a missing or failed entry stops the deployment immediately.](/blog/i-want-to-write-a-series-of-blogposts-on-how-upstash-does-ro/diagram-c52dd5b711.svg)
...
7. `helmfile apply` fires the prepare hook, which queries DynamoDB one final time

Skip or fail any step, and the deploy stops. Here is what the full journey looks like end to end:

![CI writes the test record, the tagging step links the release tag to it, and helmfile queries that same record by release tag before anything reaches production.](/blog/i-want-to-write-a-series-of-blogposts-on-how-upstash-does-ro/diagram-019e54a3f4.svg)
