...
We created all the vector databases in `us-east-1` region and created a GCP instance in `us-central1-a` region to run the benchmarks.
...
| Workload | Provider | Populate Latency | Search Latency | Recall | Population Time |
| --- | --- | --- | --- | --- | --- |
...
Populate and search latencies here are shown in 99th and 99.9th percentiles.
...
For Supabase/pgvector, we created the index before upserting the data in nq768, and created the index after upserting the data in cohere768. That’s why nq768 took significantly longer to populate the index compared to cohere768. For cohere768, index creation took another hour.
...
We calculated the cost per 1M vectors based on the number of vectors we upserted and the total cost that was reflected to us.
...
- pgvector in Supabase is cheap and highly configurable but needs careful planning (RAM, disk size, index creation timing).
