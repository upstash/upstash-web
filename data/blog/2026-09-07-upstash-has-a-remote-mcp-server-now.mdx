...
title: "The Upstash MCP Server Is Now Remote"
description: "Our MCP server is now hosted at mcp.upstash.com/mcp. Connect any client over OAuth, with no local process and no API keys in your projects."
---

Our MCP server has been around for a while, but only as a local one: your client spawned it with `npx`, and you gave it a Developer API key. That works on a laptop and nowhere else.

It is now also hosted, at a URL you connect to over OAuth:
...
## What about the CLI and the local server?
