...
AI-powered applications that respond quickly enhance the user experience by making users feel heard and seen instantly. With streaming, you can create chatbots that instantly address queries or deliver personalized recommendations in real-time. It's not just about speedâit's about meeting user needs promptly, enhancing its perceived worth.

Streaming involves sending data in small, continuous chunks rather than in one large block. In the context of AI applications, like chatbots or recommendation systems, streaming means sending partial responses to users as soon as they're available, instead of waiting to compile and deliver the entire response at once. This approach ensures that users receive immediate feedback or information, creating a more dynamic and responsive user experience.

In this guide, you will learn how to leverage Server-Sent Events (SSE) to implement streaming in Next.js endpoints with LangChain and OpenAI's language models. Further, you will learn how to cache the streaming responses with Upstash.
...
Using the OpenAI API, you are able to create chatbot responses using AI. Any request to the OpenAI API requires an authorization token. To obtain the token, navigate to the [API Keys](https://platform.openai.com/api-keys) in your OpenAI account, and click the **Create new secret key** button. Copy and securely store this token for later use as the **OPENAI_API_KEY** environment variable.

## Setting up Upstash Redis

Once you have created an [Upstash account](https://console.upstash.com) and are logged in, you are going to go to the Redis tab and create a database.
...
With the `UpstashRedisCache` class, you can initialize a Redis cache client for caching OpenAI API responses within a few lines of code. Create a file `upstashCache.tsx` inside the `app/lib` directory:
...
Using the cache handler, it becomes super easy to cache the conversations or completion responses generated by the OpenAI API. Let's move on to understand how you can start using Server-Sent Events in Next.js.
...
- The `Response` object is created with headers specific to Server-Sent Events. These headers include the following:
  ...
  - `Cache-Control: no-cache, no-transform` to prevent caching in the browser.
...
- Exports a function named `completionModel` which takes two parameters:
  ...
  - encoder: A `TextEncoder` object that encodes the messages into a `Uint8Array` object.
...
Further, create a file named `route.ts` with the following code in the `app/api/stream/completion` directory. The changes from the initial streaming route handler that you created have been highlighted below:
...
- Extracts the user message from the request body using `request.json()`.
...
Further, create a file named `route.ts` with the following code in the `app/api/stream/completion/chat` directory. The changes from the initial streaming route handler that you created have been highlighted below:
...
- Inside the `start` method of the ReadableStream, the `chatCompletionModel` function is invoked asynchronously after transforming the messages array. Each message is transformed into an array containing the role and content of the message. This function generates a streaming response from OpenAI with LangChain.
...
In this section, you will learn how to set up a minimal listener to Server-Sent Events API messages and an approach to manage the state of messages exchanged between the user and AI.

### Listening to Server-Sent Events API in React

To listen to the SSE API in our client-side component in React, you are going to initiate a POST request to the specified API route. Upon receiving a response, you would decode the incoming data as a string using the `TextDecoderStream` method, and continuously read the data from the stream.
...
### Managing the State of the Conversation in React

To manage the state of the conversation between the user and the chatbot, you can use state variables. You would update the state variables `messages` and `latestMessage` to store the conversation history and the latest message, respectively. By updating these state variables within the loop that processes incoming data from the SSE API, you can ensure real-time updates of the conversation display. The code additions to achieve the same are as follows:
...
> That was a lot of learning! Youâre all done now
