...
It has been a while since we developed the first version of the developer API where we supported creating and editing Upstash Redis databases and Upstash teams.
...
So, we have decided to put our efforts into creating a developer API that completely covers all possible actions that can be taken for any Upstash resource.

Also, it was about time we provided endpoints for monitoring Redis databases and Kafka clusters so that our customers would be able to monitor their resources and integrate with their existing
...
We are always open to any feedback, and we will make improvements based on it for sure! Your thoughts are very important to us.
