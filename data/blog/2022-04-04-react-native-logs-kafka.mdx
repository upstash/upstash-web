...
Tracking logs and errors in a software product is very essential for reliability and maintainability. Kafka is one of the most powerful solutions for collecting/storing logs and activities. Use cases can be seen in this [blog](https://blog.upstash.com/kafka-web-usescases).

In this post, we will use Upstash Kafka to collect errors and logs that occurred while users are using the mobile application which is built with React Native. Once the logs are stored in Kafka, there are endless possibilities how to consume them and integrate to log analysis tools.
...
We can create our topics under the “Topics” tab. We can configure their settings while creating them as below.
...
Lastly, we can create new credentials under the “Credentials” tab. Since our users will be the producers of logs and errors, we need to create new credentials for “produce only” purpose and permit their access for only the topics with the prefix “users.”.
...
React Native is a framework that allows developers to create native mobile apps for iOS and Android with JavaScript. Please follow the [steps](https://reactnative.dev/docs/environment-setup) to create React Native environment and start building mobile apps.

This demo app contains only one screen. On this screen, there will be four images of four different items. When a user clicks on one of them, React Native will read its price from Upstash Redis, return to the user, and send the log of this operation to Upstash Kafka. If something unexpected happens, then the app will produce the error event and send it to the “users.error” topic on Kafka.
...
Since we are demonstrating both success and failure cases, we will add prices for the first three items from the CLI. There will be no price for the last one.
...
Then, we need to configure our Kafka client to use and create a producer in the “ComponentDidMount” function, which is the first function that is executed when a screen is created.
...
In this post, we developed a mobile application using React Native, collected, and sent the logs & errors to Upstash Kafka by using the upstash-kafka [upstash-kafka](https://github.com/upstash/upstash-kafka) library, which uses HTTP requests, and used Upstash Redis to store some information related to the app.
