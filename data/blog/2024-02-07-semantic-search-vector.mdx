...
First, our system will encode the entities in our database and insert them into [Upstash-Vector](https://upstash-vector.mintlify.app/introduction) with their corresponding embeddings. Afterwards, queries will be vectorized and
the vector database will allow us to easily retrieve posts with similar embeddings. As the database uses the state-of-the-art [DiskANN](https://dl.acm.org/doi/abs/10.5555/3454287.3455520) method for the nearest neighbor search, we will enjoy
...
First, we should select a model to convert strings to semantic embeddings as this process affects the database setup. For our demonstration, we will use
...
Now that the data and a semantic encoder are ready, we should insert the data into the vector database for retrieval.

#### Creating the Vector Index

First, we will create an index using the [Upstash console](https://beta.upstash.com/vector).
...
Similarly, the similarity metric should be set to cosine similarity as this metric is supported by the model.
...
We will use the Python-SDK to connect to the created index in our application. This SDK can be installed using pip as follows:
...
Let us begin by encoding entities from StackOverflow. This can be simply done using the model from the sentence-transformers package.
...
Upon preparing our vector index, the only remaining step is to encode the query and retrieve posts with similar encodings. This is the point where using a vector database is beneficial for our project. Using the query method of the index object, we can retrieve
entities both by id and their vectors. Therefore, the search method has two steps: given a query string, encode the query and search the index for similar strings.
...
In this blog post, we demonstrated how a semantic search algorithm can be easily implemented using Upstash-Vector.
