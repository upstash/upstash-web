...
```
and then check if a user (based on his IP) has made too many requests:
```ts
const { success } = await ratelimit.limit(REQUEST-IP)
```
If success is true we let the request through, if it's false we return a 429, informing the user that he please shouldn't empty my bank account.
