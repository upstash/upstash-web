...
First, we need to setup a Redis instance for ourselves, we just need this connection string to connect our client. I'm not gonna go into detail of this just head over to [Upstash](https://console.upstash.com/redis)
and click on **Create Database**, then scroll down, and grab your connection string.
...
Let's create our Typescript project with Bun. And not because it's faster than Node, but it's much easier to setup. But, it's also pretty fast 🚀.
...
We will start off by defining our `Job` class first and visual representation of our `Queue` will be like:

So, our `Job` requires a couple of critical stuff. Status, we need to know the status of our jobs in order to
decide what to do next. Such as if we should process them or retry them maybe put them somewhere else if they are
already finished. And, of course we need to have `id` and `data` - data has to be generic so we can give our users a good user experience using types.
And, of course we need to borrow instance from the queue and the queue name.
...
This utility simply concatenates strings together. Additionally, we need to serialize our data for safe storage in Redis - we can't just pass JS objects as a data source they need to be converted into string first.
...
At this moment, we may not need these functions, but they will become crucial when we start processing jobs in the future. At that point, we'll only have the job's ID `(jobId)`, and we will need a method to reconstruct our Jobs from scratch. This is precisely what `fromId` accomplishes. It retrieves the job data from Redis, converts it into a Job instance, and returns it, so the queue can later process this job.

## Moving on to Queue

Having completed the `save()` part, let's delve into the details of Queue. Here are our objectives:
...
We have a `config`, to accept some information from outside like queue name, redis instance or retain and remove info.
We don't care about which Redis they used, although we prefer Upstash 😌 they can pass anything they want and some other info.
...
And, our class extends EventEmitter to notify them when something happens.
...
Users are aware that they have stored data like `{hello: "world"}` in the Job, but TypeScript needs assistance to provide accurate intellisense. This is why we have this mechanism in place, to inform TypeScript and facilitate a more intuitive coding experience.

"Before progressing to `jobTick()`, let's carefully consider the process:

- As our queue operates on a FIFO (First In First Out) basis, we need to start by popping a job from the queue.
...
One method is to completely wipe the entire queue, and the other is to delete a specific job from the queue.
...
- Explore running workers in service workers.
