...
In this step, we will start to create the first API endpoint, that upserts the chunks we have just created to our Upstash Vector database. To complete this task, what we have to do is:
...
To create the endpoint, we have to create a folder called `api` and create a folder that names our endpoint inside. In our case this folder's name will be `addDocuments`. For implementation of the endpoint Next.js requires the files name to be `route.js`.(`.js` part can change according to language.) 
...
We have created the necessary object, now we just have to create the embeddings and add them to our database. To do this, we will use the `UpstashVectorStore` object from Langchain. This code very simple.
...
From now on, when a user submits an input, it will parsed and upserted to the vector database. If you like, you can check whether the operation is successful or not from the `Logs` tab in Vercel or you can simply go to your relevant vector database on `Upstash Console` and check the `Data Browser` tab. Your documents will be visible there in a successful upsert.
...
This time we will handle `GET` request because search will receive its queries from the URL. For example `https://semantic-search-for-docs.vercel.app/api/search?query=computer` will search the query `computer`. (The project is deployed to vercel and named `semantic-search-for-docs` and the `prompt` variable we will declare in this section is assigned to the string `query` in this example.)
...
With that, our implementation is completed.
