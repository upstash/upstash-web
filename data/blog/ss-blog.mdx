...
Then pick your plan as you like and click `Create`. Your database is ready for you.
...
This time we will handle `GET` request because search will receive its queries from the URL. For example `https://semantic-search-for-docs.vercel.app/api/search?query=computer` will search the query `computer`. (In this example the project is deployed to vercel and named `semantic-search-for-docs` and the `prompt` variable we will declare in this section is assigned to the string `query`.)
