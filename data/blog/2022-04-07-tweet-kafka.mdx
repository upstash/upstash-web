...
Now we want to publish these tweets to a Kafka topic. We need to sign up Upstash console [here](https://console.upstash.com/auth/sign-in). Lets create Kafka cluster in Upstash. You can find instructions [here](/docs/kafka). Upstash Console already gives us the code to connect to the cluster. I just copied below connection piece off Upstash Console. We define bootstrap servers, Kafka username and Kafka password as environment variables.
