...
One cool thing is that the data is submitted via a `<form>`, so it does *not* need JavaScript to function. When JS is not available, the form will submit as normal and trigger a full page reload. When JS is available, it will update in place instead of reloading. This is accomplished using a [Svelte action](https://svelte.dev/tutorial/actions) called `enhance`, which is copied from the default SvelteKit project template. Explaining how it works is outside of the scope of this post, but take a look at the code in `src/lib/enhance.ts` if you’re curious.
