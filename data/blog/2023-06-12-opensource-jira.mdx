...
In this post, I talk about how I built an open-source alternative to Jira Kanban Board using [Upstash](https://upstash.com/), [SvelteKit](https://kit.svelte.dev/), and [Firebase Storage](https://firebase.google.com/docs/storage).
...
After you have created your database, you are then going to go to the Details tab. Scroll down until you find the Connect your database section. Copy the content and save it somewhere safe.
...
To set up, just clone the app repo and follow this tutorial to learn everything that's in it. To clone the project, run:
...
Once you have cloned the repo, you are going to create a .env file. You are going to add the items we saved from the sections above.
...
This is the main folder structure for the project. I have marked in red the files that will be discussed further in this post that deal with CRUD Operations, SvelteKit Auth, and File Upload Handler, with the files they are referenced in.
...
In this section, we'll be diving deep into how the data fetching, updating, and deletion for each issue on the Kanban board is done. We make constant use of Upstash DB (via `@upstash/redis`) to fetch, display, and refresh data.

### getTask: Fetching the issue data function

The `getTask` function uses Upstash’s `hget` via `id` as the key to make an API request to Upstash for the relevant issue data, identified by a unique `id`. If that issue is not present (or there is an error), the function is set to return an object with a `{ code: 0 }` so that then the user can be redirected to 404 (issue not found) automatically in SvelteKit’s dynamic route.
...
Using Rate Limiting, I am able to limit file uploads to up to 2 per authenticated user per task per minute. We’re able to enforce this rate limit based on the user email of the authenticated user and task’s ID. Whenever the upload gets successfully completed, we update the task in the Upstash DB with the fileURL appended to it.
...
In this section, we'll be diving deep into how an issue’s file uploads and downloads are handled in a secure and authenticated manner on SvelteKit’s edge. We leverage Firebase (v9) Storage to fetch and upload files.

### Ooh, but why not Cloudflare R2 for storage?

While I’ve seen a lot of community advocacy for Cloudflare R2’s free storage plan and its advantages, the one thing that threw me off was the need to put my credit card at Cloudflare’s disposal before even trying the system out. This made me ponder about other storage solutions, and I landed on Firebase Storage which offers me 5 GB of free storage, and in case I exceed it, my services will be stopped instead of charging my credit card without my approval and knowledge of what’s happening.
...
In conclusion, this project has provided valuable experience in implementing Granular Rate Limiting, CRUD data operations, and implementing Firebase Storage APIs to get and upload files, all of it done at the edge with Upstash’s `@upstash/redis` library!
