Here's the markdown file with grammatical corrections:

---

slug: redis-cloudflare-workers  
title: "RedisÂ® @ Edge with Cloudflare Workers"  
authors: [enes]  
...
Cloudflare Workers is the leading product in this space right now. It gives you a serverless processing environment without cold starts. You leverage Cloudflare's global network to minimize the latency of your applications. You can write your functions in JavaScript, Rust, C, and C++.

Similar to Serverless functions (AWS Lambda etc.), Cloudflare Workers are stateless. As you can see in [Cloudflareâs survey](https://workers.cloudflare.com/node), developers are asking for ways to connect their databases from Edge functions. Unfortunately, most databases are not designed for serverless environments, as they require persistent connections. We developed the REST API over RedisÂ® to enable serverless edge functions to access Upstash in the simplest and fastest way possible.
...
Now letâs write a simple example to showcase the power of the Cloudflare+Redis combination.

### Analytics at Edge

Web analytics is a very common need for all website owners. Google Analytics is powerful, but many developers do not like sharing their traffic data with Google and injecting cookies into usersâ browsers. As your website traffic comes through CDN, you can easily track your traffic at the Edge layer. Here we will implement a very simple example to showcase tracking user traffic from Cloudflare Workers. We will intercept the traffic in the Cloudflare Workers and save the user requests to Upstash Redis. Then we will write a basic standalone application which will analyze the requests and give me the following information on a selected day:
...
If you do not have one, create a database following this [guide](/docs/). On the database page, click the button REST API and copy the REST URL. If you have a Cloudflare account, you can create a Workers function or you can use [the playground](https://cloudflareworkers.com/#36ebe026bf3510a2e5acace89c09829f:about:blank) without an account too.

:::note
Select **Global Database** while creating the Upstash database. The global database replicates data to multiple regions to lower the latency from the edge functions.
:::
...
Upstash REST API requires an endpoint and a token. After creating a database, you can copy the endpoint and token from [the console](https://console.upstash.com/) by clicking the `REST API` button. Update your Cloudflare Workers function with the below code, replacing the endpoint and token:
...
Now letâs write a simple command-line application that will take the date as an argument and return us analytics data. Create a folder and run `npm init`. Then install the Redis client with `npm install ioredis`. Copy `.env.example` as `.env` file and set your Redis URL (ioredis). Update the index.js with below:
