...
## Direct uploads, without server size limits
...
**CDN delivery is included in our standard pricing.** On pay as you go, outbound bandwidth costs **$0.02 per GB** and reads cost **$0.30 per million**, whether a file is served from the CDN or origin. There is no separate CDN request or origin-transfer surcharge, and the same rates apply worldwide.

[Vercel Blob separately meters Edge Requests for every file access by URL and Fast Origin Transfer on cache misses](https://vercel.com/docs/vercel-blob/usage-and-pricing), alongside its Blob charges, subject to the plan's included allowances. Upstash Blob has no separate charges for either.
