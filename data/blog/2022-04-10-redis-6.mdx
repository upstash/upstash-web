...
Since our initial release last year, Upstash was compatible with the Redis® API up to version 5. Our Redis offering was lacking
...
We are happy to announce that Upstash Redis databases are now compatible with Redis commands up to version 6.2.

### ACL Feature

With the new release, Upstash provides an always persistent [ACL](https://redis.io/docs/manual/security/acl/) implementation.
All `ACL` subcommands available in Redis 6.2 are supported by Upstash. Note that the `ACL LOAD` and `ACL SAVE` commands are no-op
since every `ACL` update is persisted & replicated to all replicas immediately before the `ACL` command returns the response.
...
ACL RESTTOKEN <username> <password>
    ...
    The token will have the same permissions as the user.
```

You can execute the `ACL RESTTOKEN` command via `redis-cli` or CLI on the Upstash console::
...
Following is the list of new commands added with the Redis 6 release, you can find the command details
...
- [AUTH](https://redis.io/commands/auth/): The Auth command now supports an ACL `username` parameter.
- [BLPOP](https://redis.io/commands/blpop/), [BRPOP](https://redis.io/commands/brpop/), [BRPOPLPUSH](https://redis.io/commands/brpoplpush/):
  The timeout parameter is interpreted as a double instead of an integer.
- [BZPOPMAX](https://redis.io/commands/bzpopmax/), [BZPOPMIN](https://redis.io/commands/bzpopmin/):
  The timeout parameter is interpreted as a double instead of an integer.
- [LPOP](https://redis.io/commands/lpop/), [RPOP](https://redis.io/commands/rpop/): A new `count` parameter added.
- [SCAN](https://redis.io/commands/scan/): The Scan command now supports a `TYPE` parameter.
- [SET](https://redis.io/commands/set/): The Set command now supports `KEEPTTL`, `GET`, `EXAT`, and `PXAT` options.
- [ZADD](https://redis.io/commands/zadd/): The Zadd command now supports `GT` and `LT` parameters.
- [ZRANGE](https://redis.io/commands/zrange/): The Zrange command now supports `BYSCORE`, `BYLEX`, `REV`, and `LIMIT` parameters.

We have already started to work on changes coming in the Redis 7 release. Stay tuned and follow us on [Twitter](https://twitter.com/upstash)
