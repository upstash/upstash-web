...
But coupling new features or changes in general with deployment isnât always a good idea. Sometimes you follow recurring deployment schedules, but the marketing team isnât ready to announce the new feature. If the changes are hardcoded, the marketing team might need a developer to activate or deactivate them.

Feature flags, or feature toggles, are a way to decouple your changes from a deployment. You implement a change, but you hide it behind an if-statement; as long as a variable is false, the software will use the version before your change; if the variable is `true`, it will be the new version. If the variable's value comes from a database that the marketing team can access, they can toggle these changes without needing code changes.

Upstash RedisÂ® is such a database. Itâs a simple key-value store you can use to store your feature flags. Since itâs blazingly fast, it doesnât add much to your latency to check for multiple different flags in your application.

This tutorial will teach you how to build a simple feature flag mechanism for your Next.js app. We will deploy on Vercel and use Upstash RedisÂ® as storage.
...
Our first step is to create a new Next.js project. You do this with the following command:

```bash
$ npx create-next-app upstash-next-feature-flags
```

### Creating the Upstash RedisÂ® Connection

Next, we must connect our Next.js app with an Upstash RedisÂ® database. For this, we need to install the `@upstash/redis` package and create a small wrapper around it.
...
We will get the API credentials from environment variables to set them up later in this tutorial. The `flagIsActive` function will query RedisÂ® and convert the returned value to a `boolean`. This conversion is necessary because we will save the flag as a number in Redis.
...
const selectionSort = (arr) => {
  ...
  for (let i = 0; i < a.length; i++) {
    the min = a
      .slice(i + 1)
      .reduced((acc, val, j) => (val < a[acc] ? j + i + 1 : acc), i);
...
These sorting algorithms stand for any change you want to âtoggleâ in your application. Put them both into your code and switch between them via the `flagIsActive` function by changing the value in Redis.
...
After implementing everything, we need to push the project to a repository on GitHub to make it accessible for Vercel's deployment service. [GitHub explains how to create a repository in their docs](https://docs.github.com/en/get-started/quickstart/create-a-repo).

Creating an empty repository will automatically show you how you can link it with the project we created.

### Creating an Upstash RedisÂ® Database
...
As Figure 1 says, one database is free, so you wonât have to pay anything for this tutorial.
...
If you use your GitHub account to log into Vercel, adding a GitHub repository only takes a few clicks. In Figure 2, you see the top left of the Vercel dashboard; click on âAdd Newâ¦â and choose âProject.â

![Figure 2: Create a new Vercel project](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/ckg6ninjnqfu6up9jjvl.png)

Choose âContinue with GitHubâ and one of the âimportâ buttons that belong to the repository you created in the previous step.
...
When you press the big âDeployâ button, Vercel will clone your GitHub repository and deploy it on its infrastructure.
...
If we were to delete `newSortingAlgorithm` or set it to `0`, it would switch the flag back to `false`, and our API route would use the old algorithm.
...
You have to find the correct API_ROUTE_URL in Vercel. You should see a âDomainsâ category in your project on the Vercel dashboard, like in Figure 7.
...
Feature flags are an excellent way to decouple deployments from feature releases. They transform a hard-coded change into the setting of a simple value in a database, which gives more flexibility to your teams and more power to non-technical people.

With a fast database like Upstash Redis, itâs easy to implement the feature flag pattern for your application without sacrificing much of your performance. Upstash Redis has sub-second access times, and you can deploy it near your FaaS if required. All that with on-demand pricing, so you never pay for resources you donât use.