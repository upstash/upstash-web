...
We will code a REST API service that will allow client applications to request data from it (in this specific case, a list of movies); we will secure the endpoints using JWT, we will code an API login service to get the token, and we will also implement a refresh token workflow.
...
First of all, you need to sign up for [Upstash Redis](https://upstash.com/?utm_source=Lorenzo1) (a free plan will work for testing purposes), once done logging in to the console, you can create a new database:
...
We now add some dummy data using the [Upstash](https://console.upstash.com/?utm_source=Lorenzo1) CLI

![](https://miro.medium.com/max/1400/1*2SQKoOTvBC4MVzme7MIl9w.png)

We will add some movies as Redis hashes (they are basically objects) using the HMSET command:
...
We will also add a user that will be authorized to access the data; the user will also be a Redis hash:
...
Then we enter the newly created folder `upstash-jwt` and install the required modules:

```
    npm i bcrypt jsonwebtoken @upstash/redis
```

Create an `.env.local` file to store your keys and fill it with the correct data
...
Create the SECRET_TOKEN and SECRET_RTOKEN that will be used to generate the JWT. Please remember that these keys should be kept secret and should be very random/hard to guess; you can use a 64-bit to Hex string.
...
**POST /auth/signin**
It logs in the user, needing the email and password passed as a JSON object `{"email":"email", "password": "password"}`, it returns a JSON object with the user's information, a JWT, and a Refresh Token.

**GET /movies/**
Returns the list of movies as a JSON object; it requires a valid JWT that is passed in the header with this format:
_Authorization: Bearer xxx_

**GET /movies/$ID**
Returns the details of the movie with id $ID

**POST /auth/refresh**
Generates and returns a new JWT; the Refresh Token should be passed as a `refreshToken` parameter.
...
Our sign-in endpoint will only accept POST with two parameters: **user** and **password**. First, we check if the user is present in our Redis database with:
...
At this point, if the password matches, we can assume the user is authenticated and we can send back a JWT and a refresh Token; we also store the refresh token in our Redis instance.
To do so we use some functions that are in an external file called `utils.js`

It is the responsibility of the client to store the returned tokens, use them for authorization when needed, and refresh them when expired
