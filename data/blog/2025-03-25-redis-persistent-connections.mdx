...
There are different versions of TLS, so it's hard to illustrate what actually happens in the handshake, but we'll try to explain what happens in the most common version, TLS 1.2.

TLS 1.2 is a newer version of TLS that was released in 2015 and is the most commonly used version today.

The TLS handshake is slightly more complex than the TCP handshake. It is a three-way handshake between the client and the server and between the server and the client.
...
sequenceDiagram
    ...
    %% TLS 1.2 Handshake
    Client->>Redis Server: ClientHello (Supported ciphers, TLS version, Random)
    Redis Server->>Client: ServerHello (Chosen cipher, TLS version, Random)
    Redis Server->>Client: Certificate (Server's public key)
    Redis Server->>Client: ServerHelloDone
    
    Client->>Redis Server: ClientKeyExchange (Pre-Master Secret encrypted with Server's public key)
    Client->>Redis Server: ChangeCipherSpec (Start using shared secret)
    Client->>Redis Server: Finished (Encrypted handshake message)

    Redis Server->>Client: ChangeCipherSpec (Start using shared secret)
    Redis Server->>Client: Finished (Encrypted handshake message)
```
