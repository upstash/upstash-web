...
title: "Full-Stack Serverless App with Flutter, Serverless Framework, and Upstash (REDIS) - PART 1"
authors:
  - rosius
...
- low latency
- Ease of use, just like REDIS APIs.
...
where `surname` is the key and Rosius is the value.
...
Or we can retrieve all user profile information for a particular user using the `HGETALL` command like so
...
The easiest way to accomplish this is to save the userIds of every user in a list using the command LPUSH (stands for Left Push) like so
...
Here's the solution's architectural view
...
Let's get started creating our API endpoints
...
Bear in mind that we can also use the `HGET` command to get user-specific information such as firstName or lastName, etc.
...
Now, we have to grab all those ids using the command `LRANGE`.
...
After getting all userIds, we can then loop through each `id` and get user profile information using `HGETALL`.
...
I had a lot of fun writing this piece. I hope you learned a thing or two.

Did I make a mistake somewhere? A Super Saiyan like you won't hesitate to let me know.
