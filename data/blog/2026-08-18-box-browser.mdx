...
// Start your app in the box's shell
await box.exec.command("npm run dev &")
...
Here is the part a standalone browser can't do. This box starts an app, opens it in Chromium, walks a flow, and screenshots the result. Same container, start to finish.
...
// 1. Start the app in the box's shell
await box.exec.command("npm run dev &")

// 2. Open it in the box's browser
const tab = await box.browser.tab.create("http://localhost:3000")

// 3. Verify: drive the flow and capture what happened
await tab.act("complete the signup form with a test email and submit")
const png = await tab.screenshot()
```

The verification turned up a bug. The box has a coding agent looking at the same filesystem the app runs from, so it can fix the code and commit, without anything leaving the container:

```typescript
// 4. Fix: hand the problem to the agent
await box.agent.run({
  prompt:
    "The signup form accepts an empty email and still submits. " +
    "Add validation and a visible error message.",
})

// 5. Commit: git is right there too
