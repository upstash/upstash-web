...
The `timeago` library converts Unix timestamps (1636824843) into a human-readable format like `a minute ago`, `5 mins ago`, etc.
...
Don't worry about the bunny pic. It's just a placeholder for the image view.
...
If the response status code is 200, then the request was successful, we save the created UserId in shared preferences, and then we move to the home screen.
...
One of our endpoints allows a user to create a post. Here's what the screen looks like
...
Remember that, for demonstration purposes, we are using a ready-made imageUrl. In a real app, you'll have to allow a user to pick their image, upload it to a server, get the image URL, and then use it to create a post.
...
The home screen of our application displays a list of all posts created.
...
The return type of the `fetchPosts` method is a `Future<List<Post>>`.

If you run the fetchPosts() function on a slower device, you might notice the app freezes for a brief moment as it parses and converts the JSON. This is jank, and you'll want to get rid of it.
...
In the home screen file, we'll use a FutureBuilder widget to asynchronously grab all the posts as a list from the database.
...
The reason why we call fetchPosts in initState instead of the build method is that Flutter calls the build() method every time it needs to change anything in the view, and this happens surprisingly often. Leaving the fetch call in your build() method floods the API with unnecessary calls and slows down your app.
...
I'd love to see what you build next with Upstash, or how you enhance this tutorial to suit your use case.

If you found this piece helpful, please share it on your social media pages.

Have Questions? Leave a comment.

If you find an error, you know what to do. Leave a comment, and I'll be on it ASAP.
