...
The `timeago` library converts Unix timestamps (1636824843) into human-readable formats like `a minute ago`, `5 mins ago`, etc.
...
The first screen we'll be building is the create user screen, which will consume the create user endpoint.

Here's how the screen looks:

![c.png](https://cdn.hashnode.com/res/hashnode/image/upload/v1636828712136/N2CTJh_SY.png)
Don't worry about the bunny pic. Just a placeholder for the image view.
...
Here's how my final `lib` folder structure looks:
...
One of our endpoints allows a user to create a post. Here's how the screen looks:
...
`CreatePost` method looks similar to the `CreateUser` method.
...
The home screen on our application will display a list of all posts created.
...
Because fetchPosts can only return non-null values, the function should throw an exception even in the case of a â404 Not Foundâ server response. Throwing an exception sets the snapshot.hasError to true which can be used to display an error message.
...
There are still a couple of endpoints to create interfaces for, but what's a good tutorial without exercises  ð
