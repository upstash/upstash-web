...
5. The tool sends the question and current page to your server endpoint.
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return Response.json(
      { error: "Invalid documentation search request." },
      { status: 400 },
    );
  }
  ...
  try {
    const queryWithPage =
      `Current documentation page: ${parsed.data.page}\n` +
      `Question: ${parsed.data.query}`;

    // Context7 accepts queries up to 500 characters. If the page context
    // makes the query too long, preserve the user's complete question.
    const query =
      queryWithPage.length <= 500 ? queryWithPage : parsed.data.query;

    const context = await context7.getContext(query, CONTEXT7_LIBRARY_ID, {
      type: "txt",
    });
...
The tool must be registered by client-side JavaScript after the page loads. In a Next.js application, put it in a Client Component at `components/register-upstash-docs-tool.jsx`:
...
The annotations tell compatible agents that this tool only reads data and that its result comes from an external source. The registration `AbortSignal` removes the tool when the page or component is no longer active. The execution signal is passed to `fetch`, so cancelling a tool call also cancels the Context7 request.

## Make the Tool Page-Aware

The two snippets above already make the tool page-aware.

When the agent calls `search_upstash_docs`, the browser-side function reads `window.location.pathname`. If the user is reading `/redis/sdks/ts/commands/get`, the tool sends that path alongside the question:
...
The server validates both values and adds the page path to the Context7 query. The path is a relevance hint, so Context7 can prioritize documentation related to the page the user is viewing. If adding the path would exceed Context7's 500-character query limit, the endpoint sends the complete user question without the path.

The Context7 library ID remains fixed as `/upstash/docs` on the server. The browser and agent never choose it. This keeps the implementation simple and prevents callers from turning the endpoint into a search proxy for unrelated libraries.
