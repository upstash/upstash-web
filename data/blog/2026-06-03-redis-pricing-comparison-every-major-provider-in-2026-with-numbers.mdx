...
| --- | --- | --- | --- | --- | --- |
| Upstash | Redis-compatible | Per request + storage or fixed plan w/ unlimited commands | $0 | 250 MB, 500K commands/mo | Serverless, scaling, high-frequency workloads |
...
Pay-as-you-go databases charge per command and per GB of storage. The free tier covers 250 MB and 500,000 commands a month. [Pay-as-you-go](https://upstash.com/pricing/redis) is $0.20 per 100,000 commands plus $0.25 per GB-month of storage beyond the first 1 GB, which is free, and there's no bandwidth charge.
...
See the [current Upstash Redis pricing](https://upstash.com/pricing/redis) for plan limits and prices.
...
So both DigitalOcean and Upstash Redis are a good fit if you want a predictable monthly number and no Redis modules to worry about.
...
All providers with flat pricing (Upstash Redis, Redis Cloud, DigitalOcean) are easy to predict.
...
- **Upstash** if the app is serverless and/or you want the best-in-class developer experience. Also if you are using an agent to write code (Claude Code, Codex, etc.) they have the easiest time with Upstash. Pay-as-you-go is easy to start or use a fixed plan for flat, predictable pricing.
...
- **DigitalOcean** if you are already familiar with the platform and don't need modules.
...
**Is there a free Redis tier?** Upstash (250 MB, 500K commands/mo) and Aiven (1 GB) both offer free tiers with no card and no expiration. Redis Cloud has 30 MB. Google, Azure, MemoryDB, and DigitalOcean have none; AWS offers a 12-month trial node only.
