...
Upstash Workflow and Inngest share a similar approach to the problem, while Trigger.dev took a different path. They have decided to host users’ code on their server back in mid-2014 with their v3 release. You can read about their transition in their [blog post](https://trigger.dev/blog/v3-announcement#what-about-triggerdev-v2). 
...
### Free Plan 
- [Upstash Workflow Pricing](https://upstash.com/pricing): 15K steps per month. No second limit on runs, but it can be a maximum of 15K runs per month.
...
- Use-case scenario: 5M steps per month, let’s say 5 steps per run. Therefore, 1M runs.
...
Leaving the links to pricing pages here so that you can compare and run different scenarios:
...
#### [Upstash Workflow](https://upstash.com/pricing)
Upstash Workflow charges $1 per 100K steps. And the pay-as-you-go model allows 500K per day, which is 15M per month.  
This is more than our medium-traffic scenario.

Upstash Cost = 5M steps / 100K steps * $1 = $50  
Vercel Cost = $20 + Function Duration cost
...
Let’s assume a 128 MB function size and 1s duration per run.  
1M runs * 128 MB * 1s = 128 GB-hour, which is included in the Pro plan.

Total cost will be $50 + $20 = $70

#### [Inngest](https://www.inngest.com/pricing)

Inngest costs $5 per 50K runs (starts after 100K runs) and $4 per 50K steps (starts after 500K).  
There is also a base price of $50/$350. 

Inngest Cost = 1M runs / 50K runs * $5 + 5M steps / 50K steps * $4 = $100 + $400 = $500  
Vercel Cost = $20 + Function Duration cost
...
But the base cost of Inngest is already higher than Upstash Workflow.  
Let’s assume 1,000 GB-hour/month is also enough for Inngest for this case.  

Total cost will be $500 + $20 = $520 (with a warning that Vercel Function Duration Cost can be higher).
...
And when compared to the $70 predictable cost versus $48-$2,093 of Trigger.dev, Upstash Workflow would be a better choice. 

And when we compare Inngest, Upstash Workflow is also a better choice with a $70 cost compared to $520 for Inngest.
