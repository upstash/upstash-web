...
- Use-case scenarios: 
    1- 1M runs with average 5 steps per run. (5M steps)
    2- 1M runs with average 10 steps per run. (10M steps)
...
- [Trigger.dev](#trigger.dev) ->https://trigger.dev/pricing
- [Upstash Workflow](#upstash_workflow) -> https://upstash.com/pricing
...
_1M runs with average 5 steps per run_
Upstash Cost = 5M steps / 100K steps * $1 = $50  
_1M runs with average 10 steps per run_
...
Let’s assume a 128 MB function size and 0.5s duration per call.  
5M calls * 128 MB * 1s = 640 GB-hour, which is included in the Pro plan.
10M calls * 128 MB * 1s = 1280 GB-hour => 1280-1000(comes with the plan) = 280 GB-hour * $0.18 = $50.40

_1M runs with average 5 steps per run_
Vercel Cost = $20 
_1M runs with average 10 steps per run_
Vercel Cost = $20 + $50.4(Extra Function Duration cost)

Total cost will be 
- $70($50+$20) for 1M runs with average 5 steps per run
- $170.4($100+$70.4) for 1M runs with average 10 steps per run
...
But the cost of Inngest is already higher than Upstash Workflow. Let's assume same Vercel cost as Upstash Workflow with a warning.
...
- $500.4($450+$50.4) for 1M runs with average 10 steps per run
*with a warning that Vercel Function Duration Cost can be higher due to limited Rest call offloading.
