---
slug: automated-browser-testing-upstash-box
...
Some tests do not fit into CI. They need a real browser. They take twenty minutes. They clone another project's repository, compare two running servers, and when they find a problem they should open an issue with a screenshot.
...
An [Upstash Box](https://upstash.com/docs/box/overall/quickstart) with `browser: true` and a [schedule](https://upstash.com/docs/box/overall/schedules) is the third option. It is a full computer with Chromium inside. It wakes up on a cron expression, does the work, and pauses again. This post shows the setup: what goes into the box, how the test uses the browser, where the results are stored, and how we use it for Docs7.
...
<Summary>
  ...
  - We use this setup to test Docs7 against public Mintlify repositories every night. Each night it picks one repository, compares the two renderers, and opens issues for the differences it finds.
</Summary>
...
This gives you three things. The test does not need any credential, because nothing leaves the machine. Screenshots are written to the disk where the test runs, so uploading them later is a simple loop over a folder. And you control the viewport. This matters more than it sounds: the default viewport is narrow enough to switch most sites into their mobile layout, and then your test reports a missing sidebar that is not missing.

Playwright is the deterministic path. For a test that has to give the same answer every night, deterministic is what you want. The box browser also has [`act()` and `extract()`](https://upstash.com/docs/box/overall/browser/ai-actions) for the parts that are hard to script, like "find the login form on this page". They work on the same tabs, so you can mix them. For our nightly test we stayed fully scripted, because a failure on Tuesday must mean the same thing as a failure on Monday.

---

## Compare against a reference, not against your expectations

The tests that live longest do not describe how a page should look. They describe what a reference implementation does. If you build something that has to be compatible with an existing tool, run both on the same input, on two ports, and ask both the same questions.
...
Structural facts are easy to compare: heading ids, sidebar order, link targets, image counts, console errors. Pixels are not. Two renderers with different fonts and spacing will never match pixel by pixel, and a pixel diff would flag every page every night. Take screenshots as evidence for a human, and run the assertions on the DOM.
...
**State goes to Redis.** The queue of things to test, what was tested and when, and one record per run. The most useful key is the fingerprint map. Every failure gets a stable id made from the check name and a failure class, for example `HEADING-IDS/punctuation-stripped`, and Redis remembers which issue number belongs to it. This is what stops the second night from filing the first night's bug again.

**Issues go to GitHub, from inside the box.** A `box git clone` of the target repository puts the account's GitHub credential into the box, and `gh` is already installed. So the last step of the run can list the open issues, match them by a marker in the body, comment on the ones it already knows, and create the rest. Set a limit. Three new issues per night is enough. A bot that opens twenty issues in one night gets muted.

---

## Building it without SSH

Everything above was built and debugged from an editor session through the [Upstash MCP server](https://upstash.com/docs/box/guides/remote-development): create the box, run commands, open a page in the box browser and look at it, create the schedule, mint the upload URLs, pause the box. When a run failed at three in the morning, reading the box's system log and the run history was one tool call each. The box is the machine. MCP is the keyboard.
...
A queue in Redis holds public repositories that contain a Mintlify `docs.json`, sorted by stars. Every night the box picks the next one, clones it at the current commit, starts `docs7 dev` on one port and Mintlify's own `mint dev` on another port, and walks the same pages on both: sidebar order, active item, heading ids, table of contents, every internal link and anchor, images, code blocks, console errors, theme, fonts, the repository's own custom CSS, and the generated `llms.txt`. About three hundred pages fit into the forty-minute budget. Differences are fingerprinted, screenshot pairs are uploaded, and the last step files new issues or comments on existing ones.
