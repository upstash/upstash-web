...
```tsx title="hooks/use-debounce.ts" {1,3-5} use-debounce.ts
...
```tsx {12-20} title="components/search-bar.tsx"
"use client";
...
## 2. Sending Search Query to our Backend
...
When we enter a query, we now see that a single (because of the debounce) request for our term is made to our backend:
