...
In this article, I will create a basic API which will count the page views per each edge location. I will use [Upstash Global Redis](/docs/redis/features/globaldatabase) to keep the page counts. Because Upstash replicates the data to multiple regions, it helps to keep the latency low. I will deploy the Next.js application to Vercel which supports the Edge runtime.
