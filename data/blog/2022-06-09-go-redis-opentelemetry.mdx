...
In this tutorial, you will learn how to connect to an Upstash Redis database using the go-redis client and monitor the performance of your app using distributed tracing.
...
To connect to an Upstash Redis database, use the following code:
...
To instrument the Redis client, you need to add the hook provided by redisotel:
...
To make tracing work, you must pass the active [trace context](https://opentelemetry.uptrace.dev/guide/go-tracing.html#context?utm_source=upstash) to the go-redis commands, for example:
