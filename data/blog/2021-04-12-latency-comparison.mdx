...
In this article, I will compare the latencies of three serverless databases: DynamoDB, FaunaDB, and Upstash (Redis) for a common web use case.

I created a sample news website and am recording database-related latency with each request to the website. Check [the website](https://news-app-two-omega.vercel.app/) and [the source code](https://github.com/upstash/latency-comparison).

I have inserted 7,001 NY Times articles into each database. The articles are collected from New York Times Archive API(all articles of January 2021). I randomly scored each article. At each page request, I query top 10 articles under the `World` section from each database.

I use serverless functions (AWS Lambda) to load the articles from each database. The response time of fetching 10 articles is being recorded as latency inside the lambda function. Note that the recorded latency is only between the lambda function and the database. It is not the latency between your browser and the server.
...
I created a DynamoDB table in US-West-1 with the read and write capacity of 50 (the default value was 5).
...
FaunaDB is a globally replicated database, afaik there is no way to select a region. I used FQL, assuming the GraphQL API may have some overhead.
...
I created a Standard type database in the US-West-1 region on Upstash. I used a Sorted Set per each news category. So all `World` news articles will be in the Sorted Set with key `World`.

### Initialize Databases

I downloaded the 7,001 news articles as a JSON file from the NYTimes API site, then created a NodeJS script for each database which reads the JSON and inserts the news record into the database. See the files: [initDynamo.js](https://github.com/upstash/latency-comparison/blob/master/initnewsdb/initDynamo.js), [initFauna.js](https://github.com/upstash/latency-comparison/blob/master/initnewsdb/initFauna.js), [initRedis.js](https://github.com/upstash/latency-comparison/blob/master/initnewsdb/initRedis.js)
...
On [FaunaDB’s status page](https://status.fauna.com/), you will see latency numbers in the hundreds. So I assume my configuration does not have big flaws. There can be two reasons behind this latency difference:
...
There was an active discussion on [HackerNews](https://news.ycombinator.com/item?id=26799074) about my benchmark and the performance of Fauna. I applied the suggestions and restarted FaunaDB application. That’s why, the number of FaunaDB records in the histogram is less than others.
