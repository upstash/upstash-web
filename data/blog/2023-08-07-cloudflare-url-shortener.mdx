---
slug: cloudflare-url-shortener
title: "Build URL Shortener with Cloudflare - Upstash Integration"
authors:
    - fahreddin
...
-   Create the Worker functions on Cloudflare
...
Executing this command will lead you through the options and configurations for the function. Finally, it will request you to log in to Cloudflare. Below is an example of my configuration and the resultant output:
...
We've created and deployed the function using a single line command. This command should have initialized a folder similar to this:
...
Access to the [Cloudflare Dashboard](https://dash.cloudflare.com) and login with the same account that you've created the `url-shortener` function. Then, navigate to the **Workers & Pages > Overview** section of the sidebar. Here, you will find the `url-shortener` function listed.

![Cloudflare workers](/blog/cloudflare-url-shortener/cloudflare-workers.png)

Clicking on the function will direct you to the function details page, where we will perform the integration process. Switch to the **Settings** tab in the function details, and proceed to **Integrations** section. You will see various Worker integrations listed. To proceed, click the **Add Integration** button associated with the Upstash Redis.

![Add integration](/blog/cloudflare-url-shortener/add-integration.png)

On the Integration page, connect to your Upstash account. Then, select the `url-shortener` database from the dropdown menu. Finalize the process by pressing **Save button**.

**Note:** Please don't make any changes in the secret names on the **Configure secrets** step.
...
-   When user accesses the Worker URL with previously added key as a path, corresponding value of the key will be retrieved from Redis. The user will then be redirected to this URL with a `301` response code.

We will implement the function in the `src/worker.ts` file. Below is the basic implementation of our URL Shortener:
...
Now that our algorithm is ready, there's only one step left: deploying the function. Navigate to `url-shortener` folder from the terminal, and run the command below:

```shell
npm run deploy
```

Our Cloudflare Worker is now up and running! You can test the function either in the function dashboard, or via a curl request. Here's an example request for you. You can replace the URL with your Worker URL, or test with my implementation:
