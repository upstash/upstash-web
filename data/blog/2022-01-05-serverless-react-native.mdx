...
"Sorted Set", which is supported by Redis, is a sorted data type that enables users to perform storing, adding, removing, and querying by range operations in a very fast manner.

A sorted set is exactly what is needed for storing, updating, and displaying a sorted leaderboard.
...
Let's start by installing and configuring the Serverless Framework for AWS. Visit and follow the steps in the [Serverless quick start](https://www.serverless.com/framework/docs/providers/aws/guide/quick-start).
...
First, we need to import and configure Redis, which is the only dependency we have. To add the Redis dependency to the Serverless framework, we need to add the "Serverless Python Requirements" plugin. Run the command,
...
By using the zadd function of Redis, we can add users and scores to the sorted set "Leaderboard". Example:
...
Our second function is getLeaderboard. This function accepts GET requests from users and returns the leaderboard in descending order, which is read from Redis.
...
In our mobile application, there will be two screens. The first one is the screen where users add a new score along with user information.
...
undefined"Score added successfully!", then this means that the request we sent is received and the score is added successfully.
