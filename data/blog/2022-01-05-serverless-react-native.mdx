...
In a typical leaderboard app, user information and scores belonging to users need to be stored. Since all of this data should be sorted by scores, using Redis is one of the best possible solutions.

âSorted Set,â which is supported by Redis, is a sorted data type that enables users to perform storing, adding, removing, and querying by range operations in a very fast manner.

Sorted set is exactly what is needed for storing, updating, and displaying a sorted leaderboard.

#### **1.1 - Getting Started with Upstash**

Upstash provides a serverless database for Redis. For more detailed information about how Redis works, please check the [Redis](https://redis.io/documentation) documentation.
...
- Free tier for storage and operations
...
To get familiar, we can perform some operations in CLI, which is provided in the Upstash console.

First, we can start our database by adding a new user with a score to the sorted set, which we name as Leaderboard.
...
We can perform these operations on AWS Lambda functions with the Serverless Framework to connect Redis to the backend of the app.

### **2 - Create Functions with Serverless Framework**

Serverless is a framework that allows us to work with serverless functions of cloud providers such as AWS, Azure, Google Cloud, etc. It is a very powerful tool to implement and manage serverless functions from the user side.

Letâs start by installing and configuring the Serverless Framework for AWS. Visit and follow the steps at [Serverless quick start](https://www.serverless.com/framework/docs/providers/aws/guide/quick-start).

After installation, we have handler.py and serverless.yml.

- serverless.yml

  In this file, we will define the functions that we will implement. In our case, we will only need to add new users and get the leaderboard to display. Therefore, defining âaddScoreâ and âgetLeaderboardâ functions should be enough.
...
- handler.py
  ...
  In this file, we will implement the functions, which are the codes that will be executed in the backend when an HTTP request is sent by a mobile app as defined in the serverless.yml file.

  First, we need to import and configure redis, which is the only dependency we have. To add the Redis dependency to the Serverless Framework, we need to add the âServerless Python Requirementsâ plugin. Run the command,
  ...
  The first one is adding new users and scores to the leaderboard. This is a POST request. Users will send their information inside the body of the HTTP request.
  ...
  By using the zadd function of Redis, we can add users and scores to the sorted set âLeaderboard.â Example:
...
React Native is a framework that allows us to develop mobile applications for multiple platforms by writing code in JavaScript.
...
The screen where users submit scores looks like below.
...
In this screen, when a user enters a score, the application will send an HTTP request to our serverless endpoint
...
As you can see, the POST request body contains the keys âfirstnameâ, âlastnameâ, and âscore,â and the corresponding values that we get from users.
...
Now, we will design a very simple leaderboard screen. The user can be navigated to the leaderboard screen by clicking the âGo to Leaderboardâ button.

The Leaderboard screen will look like this.
...
Complete source code of the application is available at [upstash-react-native-project](https://github.com/upstash/redis-examples/tree/master/react-native-leaderboard)

### **Conclusion**

In this post, we developed a mobile application for the leaderboard which is backed by Python functions running on AWS Lambda through the Serverless Framework. We stored our leaderboard inside Upstash Redis.
