...
Serverless Redis® is a fully managed database-as-a-service product where the pricing is based on per command, so you are only charged for what you actually use.
...
Of course, you can also use your database to store the user sessions and cache data, but for better performance, it is recommended to use Redis®. If you want to learn more about the performance benefits of the different options, check out this great article here: [Which is the best Laravel cache driver for performance?](https://www.georgebuckingham.com/laravel-cache-driver-performance/).
...
- Horizontal scaling, on the other hand, is when you add more servers that are responsible for serving requests to **scale-out**.
