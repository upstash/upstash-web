...
Okay, now we need a way to limit the number of emails that users are allowed to send. That's where Upstash comes in with its magic library: `@upstash/ratelimit`. Let's create an account on the [Upstash Console](https://console.upstash.com/) and grab our keys: `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`.
