...
Vercel KV is an invaluable tool for web projects, yet its extensive use can lead to a rapid increase in the number of HTTP requests, potentially impacting performance. While pipelines offer a way to batch commands and reduce these requests, they can be cumbersome to implement. Is there a solution that combines the efficiency of pipelines with the simplicity of basic Redis commands? In comes ***auto-pipelining*** – offering a seamless way to enhance performance without the coding complexity.
...
Upon calling `deferExecution`, the command yields control of the Node main thread. The next `GET` command in the sequence then gains control of the thread and proceeds with its execution, doing exactly the same thing as the first `GET`: Adding itself to the active pipeline and yielding the control of the thread.
  ...
  activePipeline = activePipeline || createNewPipeline();
	...
	const pipelinePromise = deferExecution().then(() => {
      ...
      commandIndex = 0
		...
		return pipelinePromises.get(pipeline)!;
	});
...
The v0 team initially shipped auto-pipelining as a hack for themselves. Now, the code has been upstreamed in Upstash’s Redis client version 1.31.3, allowing everyone to benefit from it by activating a simple option in the Redis client. Additionally, it’s now enabled by default in Vercel KV version 2.0.0.