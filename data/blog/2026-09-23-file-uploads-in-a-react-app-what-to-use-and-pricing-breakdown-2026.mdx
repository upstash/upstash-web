...
- **An app that runs fully on Vercel:** Vercel Blob works, but it charges $0.05 per GB served, while Upstash Blob serves the first 1 TB a month for free.

## How much do file uploads cost?

File upload costs depend on what you pay per GB stored each month and what you pay per GB your users download (egress). Upstash Blob charges [$0.02 per GB stored and nothing for the first 1 TB of egress a month](https://upstash.com/pricing/blob), Vercel Blob charges $0.023 and $0.05 respectively, and Cloudflare R2 charges $0.015 with free egress.

| Service | Storage per GB-month | Egress per GB | Free tier |
| --- | --- | --- | --- |
| [Upstash Blob](https://upstash.com/pricing/blob) | $0.02 | Free up to 1 TB a month, then $0.02, CDN included | 1 GB storage, 10 GB bandwidth, no credit card |
...
```text
Upstash Blob:        100 × $0.02  + 1,000 × $0     = $2.00
...
R2 and Upstash Blob are the cheapest because neither charges for this much egress: R2 never does, and Upstash Blob serves the first 1 TB a month for free. On R2 you write presigned URLs, progress, multipart and retries yourself, while Upstash Blob includes the upload flow for 50 cents more.
