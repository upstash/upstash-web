...
Redis already comes with two great commands that make it trivial to deduplicate
and to increment a counter.

In order to get a more accurate counter, I want to debounce the incrementing of the
...
Now that we have a Redis database, we can start implementing the counter. First, we need to install @upstash/redis:
...
Upstash and `@upstash/redis` are compatible with Vercel's edge functions, so
...
Next, we'll require a slug or similar identifier to be passed in the request
...
Tracking the views is nice, but we also want to display them publicly. Let's see how we can achieve that.

In order to display the number of views, we need to look them up from the database. We can achieve this by using the `GET` command. We should also add a `revalidate` config to the page component, so that the page is revalidated every 60 seconds and not for every request.
