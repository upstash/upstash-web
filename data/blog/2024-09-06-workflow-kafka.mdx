...
As we focus on this new direction, we have made the difficult decision to discontinue support for Upstash Kafka starting in six months. Until then, we will fully support Kafka with no changes to allow easy migration; we will just not accept new users. This choice will enable us to dedicate our resources and focus entirely on developing and improving QStash and Upstash Workflow to become the go-to tools for building reliable serverless applications.

We understand that our shift away from Kafka can be challenging, and we are committed to supporting every Kafka user through this transition with our professional support and migration guides. In the following sections, I will provide more details about Upstash Workflow's capabilities and our plan for phasing out Upstash Kafka.
...
3. Extended Execution Time: Each step can take up to a maximum execution duration of QStash and supports hour-long HTTP requests.
...
- **Data Pipelines**: Process complex data, such as cleaning, transforming, and loading data from multiple sources. QStash Workflow ensures that each step in your pipeline executes correctly and that data is processed reliably without your needing to manage any infrastructure.
