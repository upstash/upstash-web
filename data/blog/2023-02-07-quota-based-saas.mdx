...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // If this is the first time the user makes an API request the returned value
  // will be 1 and we set the expiration to 1 day.
  if (quota === 1){
...
If you want to see this kind of optimisation live, you can check out [**fastsheet**](https://fastsheet.co/), an API services that turns your Google Sheets into an API in a few clicks. It's free to use with a generous free tier.
