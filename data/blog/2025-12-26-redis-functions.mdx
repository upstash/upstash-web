...
authors: [josh, yusuf]
tags: [redis]
description: "Upstash Redis SDK now supports Redis Functions. Like stored procedures for Redis, with full TypeScript support."
---
...
// 👇 we use `callRo` for read-only functions
const value = await redis.functions.callRo("get_value", ["mykey"], []);
```

---

That's it. If you've been running Lua scripts with `EVAL`, this is a cleaner way to organize and reuse that logic. Appreciate you for reading 🙌