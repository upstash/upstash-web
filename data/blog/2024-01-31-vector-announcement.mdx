...
Today, we're introducing Upstash Vector, a serverless vector database built for working with vector embeddings for AI models and LLMs. Upstash Vector will help you to efficiently store and query high-dimensional vector embeddings. Upstash Vector operates on a serverless model, abstracting away hosting and management complexities.
...
Although storage and managing vector data is straightforward, querying for vector similarity poses significant challenges.

A simple and basic approach is to do an exhaustive search; by comparing a query vector to every other vector stored in the database one by one. But this consumes too much resources and results in very high latencies, hence not very usable. To address this problem, _Approximate Nearest Neighbor_ (ANN) algorithms are used. ANN search approximates the true nearest neighbor. This means it might not find the absolute closest point, but it will find one that's _close enough_ with much faster performance and consuming fewer resources.
...
Upstah Vectors allows storing `JSON` metadata alongside the vector embeddings. So you can insert embedding-related metadata and get them back while querying the database. Upstash Vector currently supports storing and retrieving metadata alongside vectors. 
