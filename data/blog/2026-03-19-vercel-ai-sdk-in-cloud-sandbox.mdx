...
<Note>
  A note on security: In this example, the user's prompt is interpolated
  directly into a shell string. I think this example gets the point across well,
  but in production you'll want to avoid this. Instead, we can write the prompt to
  Upstash Redis and read it from inside the sandbox (that way user input never
  touches the shell).
</Note>
