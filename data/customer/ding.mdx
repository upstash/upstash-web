...
highlight: "With Upstash we minimized our operational costs and we also felt very safe that we only pay for what we use, this is on top of the amazing support we get whenever we need it."
...
Ding's infrastructure mainly utilizes Go and runs on AWS Lambda, achieving complete serverlessness across all operations. They rely on DynamoDB for OLTP (Online Transaction Processing) and Redshift for OLAP (Online Analytical Processing) workloads. Upstash plays multiple roles in this architecture, serving both as a caching solution, enabling rate limiting, and providing simple database store functionality due to its durability feature.
...
Moreover, Upstash Redis's rate limiting is a key component of Ding's anti-fraud algorithms, helping in the early detection of fraudulent behavior signals and preventing fraudulent authentications and saving costs for their users
